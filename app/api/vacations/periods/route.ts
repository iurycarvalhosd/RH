import { type NextRequest } from "next/server";
import { z } from "zod";
import { createAdminClient } from "@/lib/supabase/admin";
import { requireProfile } from "@/lib/auth/rbac";
import { apiErrorResponse, getBranchFilter, jsonOk } from "@/lib/api-helpers";
import { computeVacationStatus } from "@/lib/calculations/vacations";
import { getAppSettings } from "@/lib/calculations/settings";
import type { VacationPeriod } from "@/lib/types";

const PeriodInput = z.object({
  employee_id: z.string().uuid(),
  period_start: z.string().min(1),
  period_end: z.string().min(1),
  due_date: z.string().min(1),
  days_available: z.number().nonnegative().default(30),
});

export async function GET(request: NextRequest) {
  try {
    await requireProfile();
    const branchId = getBranchFilter(request);
    const employeeId = request.nextUrl.searchParams.get("employeeId");

    const admin = createAdminClient();
    let query = admin
      .from("vacation_periods")
      .select("*, employee:employees!inner(id, name, branch_id)")
      .order("due_date");
    if (employeeId) query = query.eq("employee_id", employeeId);
    if (branchId) query = query.eq("employee.branch_id", branchId);

    const { data, error } = await query;
    if (error) throw error;

    const settings = await getAppSettings();
    const rows = (data as VacationPeriod[]).map((p) => ({
      ...p,
      status: computeVacationStatus(p.due_date, settings.vacation_alert_days),
    }));
    return jsonOk(rows);
  } catch (e) {
    return apiErrorResponse(e);
  }
}

export async function POST(request: NextRequest) {
  try {
    await requireProfile();
    const body = PeriodInput.parse(await request.json());
    const admin = createAdminClient();
    const { data, error } = await admin.from("vacation_periods").insert(body).select().single();
    if (error) throw error;
    return jsonOk(data, { status: 201 });
  } catch (e) {
    return apiErrorResponse(e);
  }
}
